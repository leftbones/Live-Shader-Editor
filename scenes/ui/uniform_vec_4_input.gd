...
@onready var input_w: LineEdit = $InputW
...
func _ready() -> void:
	...
	input_w.text_changed.connect(func(new_value: String):
		value_changed.emit(label.text, value)
	)
...
func init(uniform_name: String, uniform_value: Vector4) -> void:
	...
	input_w.text = str(uniform_value.w)